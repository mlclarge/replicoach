...
export default async function handler(req, res) {
  ...
  if (downloadError || !fileData) {
    return res.status(500).json({ error: 'Erreur téléchargement source', details: downloadError });
  }
  ...
  if (uploadError || !uploadData?.path) {
    return res.status(500).json({ error: 'Erreur upload destination', details: uploadError });
  }
